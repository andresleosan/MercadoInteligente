...
- **Migración:** Originalmente planificado para Firebase Hosting (v1). Migrado a Cloudflare Pages por mayor ancho de banda y build automático desde Git. `firebase.json` se mantiene para Firestore rules y emulators; la sección `hosting` ya no se usa para deploy.
- **Por qué:** Cloudflare Pages es gratuito con ancho de banda ilimitado y CI/CD nativo desde GitHub, sin configuración de workflows manual. Firebase sigue siendo el backend de Auth/Firestore; el almacenamiento de tickets vive en Supabase Storage.
