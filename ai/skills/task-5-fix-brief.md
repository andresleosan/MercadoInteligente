# Task 5 Fix: 3 issues from code review

## Issue 1 (Critical): Restore dashboard-multi-month.test.tsx

The integration test file `src/tests/integration/dashboard-multi-month.test.tsx` was deleted but the plan said to rewrite it. Restore it with this content:
...
In the test "should show resumen with presupuesto and restante when budget exists", ensure `screen.getByText('Gastado')` assertion is present inside `waitFor`.

## Issue 3 (Important): Put assertions inside waitFor

Move the `getAllByText('Presupuesto').length` assertion inside `waitFor` to avoid race conditions.

## Steps

- [ ] Create dashboard-multi-month.test.tsx with the content above
- [ ] Fix Dashboard.test.tsx to include Gastado assertion and proper waitFor wrapping
- [ ] Run both test files: `npx vitest run src/pages/Dashboard.test.tsx src/tests/integration/dashboard-multi-month.test.tsx`
- [ ] Amend the commit or create a new one:

```bash
git add src/pages/Dashboard.test.tsx src/tests/integration/dashboard-multi-month.test.tsx
git commit -m "fix: restaurar integration test, corregir asserts segun review"
```
